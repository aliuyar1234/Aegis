...
MANIFEST_SCHEMA = json.loads((ROOT / "schema/jsonschema/phase25-ga-transition-evidence-manifest.schema.json").read_text(encoding="utf-8"))
BUNDLE_SCHEMA = json.loads((ROOT / "schema/jsonschema/phase25-ga-transition-evidence-bundle.schema.json").read_text(encoding="utf-8"))
...
def main(argv: list[str]) -> int:
    ...
    manifest = load_yaml("meta/phase25-ga-transition-evidence-manifest.yaml")
    ...
    if not all((ROOT / rel).exists() for rel in manifest["required_gates"]):
        raise SystemExit("PHASE-25 evidence manifest references missing gates.")
    for group in manifest["evidence_groups"]:
        if not all((ROOT / rel).exists() for rel in group["refs"]):
            raise SystemExit(f"PHASE-25 evidence group {group['id']} references missing assets.")
