...
MANIFEST_SCHEMA = json.loads(
    (ROOT / "schema/jsonschema/ecosystem-evidence-manifest.schema.json").read_text(encoding="utf-8")
)
BUNDLE_SCHEMA = json.loads(
    (ROOT / "schema/jsonschema/ecosystem-evidence-bundle.schema.json").read_text(encoding="utf-8")
)
REPORT_COMMANDS = [
    ["scripts/run_phase18_certification.py"],
    ["scripts/run_phase18_sandbox_profiles.py"],
    ["scripts/run_phase18_policy_bundles.py"],
    ["scripts/run_phase18_public_compatibility.py"],
]
...
def main(argv: list[str]) -> int:
    ...
    manifest = load_yaml("meta/ecosystem-evidence-manifest.yaml")
