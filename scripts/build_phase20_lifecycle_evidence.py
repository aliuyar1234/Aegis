...
MANIFEST_SCHEMA = json.loads(
    (ROOT / "schema/jsonschema/lifecycle-evidence-manifest.schema.json").read_text(
        encoding="utf-8"
    )
)
BUNDLE_SCHEMA = json.loads(
    (ROOT / "schema/jsonschema/lifecycle-evidence-bundle.schema.json").read_text(
        encoding="utf-8"
    )
)
REPORT_COMMANDS = [
    ["scripts/run_phase20_operator_accreditation.py"],
    ["scripts/run_phase20_rollout_waves.py"],
    ["scripts/run_phase20_incident_reviews.py"],
    ["scripts/run_phase20_deprecation.py"],
]
...
def main(argv: list[str]) -> int:
    ...
    manifest = load_yaml("meta/lifecycle-evidence-manifest.yaml")
