...
| Test paths/commands incoherent | High | Fixed | `meta/test-suites.yaml`, `test/`, `tests/`, `docs/overview/test-strategy.md` | Runtime implementation tests remain scaffolded by design |
| Acceptance criteria phase-leaky / weak | High | Fixed | `meta/acceptance-criteria.yaml`, phase docs, `docs/overview/acceptance-criteria.md` | None |
...
| Security/policy too prose-heavy | High | Fixed | security catalogs, JSON Schemas, `docs/operations/security.md`, `docs/design-docs/security-governance.md` | None |
...
| Phase gates were typed too loosely to serve as strong execution contracts | High | Fixed in v3 | `schema/jsonschema/phase-gate.schema.json`, `tests/phase-gates/*.yaml`, `scripts/run_phase_gate.py`, `docs/overview/test-strategy.md` | Phase gates remain contract-level gates, not a live implementation runner |
