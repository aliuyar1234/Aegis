defmodule Aegis.Runtime.SessionKernelTest do
  use ExUnit.Case, async: false

  alias Aegis.Events
  alias Aegis.Runtime
  alias Aegis.Runtime.OwnershipGuide
  ...
  test "current phase starts on session kernel" do
    assert File.read!(Path.join(@repo_root, "meta/current-phase.yaml")) =~ "PHASE-01"
  end

  test "runtime model doc exists" do
    assert File.exists?(Path.join(@repo_root, "docs/design-docs/runtime-model.md"))
  end

  test "ownership guide exposes the golden process map and example flow" do
    process_owners = OwnershipGuide.process_owners()
    golden_example = OwnershipGuide.golden_example()

    assert Enum.any?(process_owners, &(&1.process == Aegis.Runtime.SessionKernel))
    assert Enum.any?(process_owners, &(&1.process == Aegis.Runtime.ToolRouter))

    assert Enum.map(golden_example.commands, &elem(&1, 0)) == [
             :activate,
             :request_action,
             :wait,
             :change_control_mode,
    ...
    assert golden_example.expected_projection.phase == "waiting"
    assert golden_example.expected_projection.control_mode == "supervised"
  end
